            ...
            <div key={index} className="pt-4 first:pt-0 space-y-3">
              ...
              <div className="mt-2 text-[10px] font-mono text-zinc-500 uppercase">
                <details className="cursor-pointer group">
                  ...
                  <ul className="mt-2 space-y-2.5 bg-zinc-950/40 p-3 rounded border border-zinc-900/60 font-sans normal-case text-xs text-zinc-300">
                    {scene.playerTasks.map((task, pIdx) => (
                      <li key={pIdx} className="leading-relaxed border-b border-zinc-900 last:border-0 pb-2 last:pb-0">
                        <strong className="text-zinc-400 font-mono text-[10px] block mb-0.5">{task.characterName} ({task.concreteActionCategory}):</strong>
                        <div className="space-y-1 pl-2 border-l border-zinc-800">
                          {task.instructionPrefix && <p className="text-zinc-400">{task.instructionPrefix}</p>}
                          {task.dialogueLines && task.dialogueLines.length > 0 && (
                            <p className="font-bold text-white">🗣️ "{task.dialogueLines.join('", "')}"</p>
                          )}
                          {task.instructionPostfix && <p className="text-zinc-500 italic text-[11px] font-sans">{task.instructionPostfix}</p>}
                        </div>
                      </li>
                    ))}
                  </ul>
