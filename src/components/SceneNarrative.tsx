...
import { Film, Milestone, RefreshCw, ArrowRight } from "lucide-react";
...
export function SceneNarrative({ scene, totalScenes, onNextScene, isGenerating = false }: SceneNarrativeProps) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5 md:p-6 shadow-xl space-y-5 animate-fade-in relative overflow-hidden">
      ...
      {/* Narrative block with next scene button integrated side-by-side on wide screens */}
      <div className="flex flex-col lg:flex-row gap-5 lg:items-center justify-between">
        <div className="space-y-3 flex-1 min-w-0">
          <span className="inline-flex items-center gap-1.5 text-xs font-mono tracking-widest uppercase font-bold text-red-500/90 leading-tight">
            <Milestone className="w-3.5 h-3.5 text-red-500 animate-pulse" />
            Tilannekuvaus:
          </span>
          <blockquote className="border-l-4 border-red-500 pl-5 py-2 text-zinc-100 text-lg md:text-xl lg:text-2xl font-serif italic tracking-wide leading-relaxed bg-zinc-950/45 rounded-r shadow-inner p-4">
            “{scene.narrativeIntroduction}”
          </blockquote>
        </div>

        {onNextScene && (
          <div className="shrink-0 w-full lg:w-auto flex items-center justify-center lg:self-stretch lg:border-l lg:border-zinc-850 lg:pl-5">
            <button
              type="button"
              onClick={onNextScene}
              disabled={isGenerating}
              className={`w-full lg:w-56 h-full py-4 lg:py-0 px-6 rounded-xl font-mono text-xs sm:text-sm font-black uppercase border tracking-wider flex items-center justify-center gap-2.5 transition duration-300 shadow-2xl select-none min-h-[64px] lg:min-h-0 ${
                isGenerating
                  ? "bg-zinc-950 border-zinc-900 text-zinc-500 cursor-not-allowed"
                  : "bg-red-650 hover:bg-red-500 text-white border-red-500/20 hover:scale-[1.03] active:scale-[0.97] cursor-pointer"
              }`}
            >
              {isGenerating ? (
                <>
